...
src/maths_parser_detroix23/parsing/split.py

Find where to "split" the token list. 
"""

from maths_parser_detroix23.structures import types, tokens, operators

def first_priority(token_list: list[tokens.Token | types.Number]) -> int:
	"""
	Choose the first operator from `token_list`, skips if next operator has <= priority.  
...
def choose(token_list: list[tokens.Token | types.Number]) -> int:
	"""
	...
	1. (**Current**) Choose the first operator, skips if next operator has <= priority.
	2. Choose the first irreducible, an operation between plain numbers:
	...
	return first_priority(token_list)
